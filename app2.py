...
@st.cache_data
def carregar_dados():
    planejado = pd.read_excel("dados/dados.planejado.xlsx", engine="openpyxl")
    realizado = pd.read_excel("dados/dados.realizado.xlsx", engine="openpyxl")
...
pagina = st.sidebar.radio(
    "📄 Navegação",
    ["Resumo END", "Detalhamento END", "Condições Físicas"]
)
...
# =====================================================
# FUNÇÃO GRÁFICO ROSCA (ALTAIR)
# =====================================================
def grafico_rosca(df, titulo):
    escala_cores = alt.Scale(
        domain=["Planejado", "Realizado"],
        range=["#006400","#7CFC00" ]  # verde-limão / verde-escuro
    )
    ...
    texto = base.mark_text(radius=95, size=14, color="black").encode(
        text="Quantidade:Q"
    )

    return (pie + texto).properties(title=titulo)
...
if pagina == "Resumo END":
    ...
    st.altair_chart(
        grafico_rosca(df_rosca, "Planejado × Realizado – END"),
...
if pagina == "Detalhamento END":
    ...
    for i, (ensaio, (col_p, col_r)) in enumerate(ensaios.items()):
        df = pd.DataFrame({
            ...
            "Quantidade": [
                planejado_f[col_p].fillna(0).sum(),
                realizado_f[col_r].fillna(0).sum()
            ]
        })

        grid[i].altair_chart(
            grafico_rosca(df, ensaio),
            use_container_width=True
        )
